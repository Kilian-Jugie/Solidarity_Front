...
function historiqueMessage() {
  fetch(
    "http://localhost:3000/api/users/" + JSON.parse(document.cookie)["user"]
  )
    .then((res) => res.json())
    .then((result) => {
      alert(result["ID"]); //3
      ...
      .then((result) => {
        ...
        fetch(
          "http://localhost:3000/api/users/" +
            JSON.parse(document.cookie)["user"]
        )
          .then((res) => res.json())
          .then((result) => {
            alert(result["ID"]); // renvoie 3
            fetch("http://localhost:3000/api/messages/" + result["ID"], {
              ///messages/{id l'expediteur}
              method: "POST",
              headers: {
          ...
          <section>
            ...
            {this.state.activeTab === 1 && (
              <div>
                ...
                <button onClick={historiqueMessage}>refresh</button>
                <ul>
                  <li>{Donner}</li>
                </ul>
              </div>
